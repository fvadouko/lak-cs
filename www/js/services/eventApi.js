...
const setEventPointed = (e_id)=>{
    ...
    EVENTS.loadDatabase((err)=>{
        if(err){
            console.log(`From eventApi.setEventPointed error in EVENTS.loadDatabase: ${err}`);
        };       
    });

    EVENTS.update({_id:e_id},{$set:{pointed:true}},{},(err)=>{
        if(err){
            console.log(`From eventApi.setEventPointed error in EVENTS.update: ${err}`);
        }; 
    });
}
...
const eventsNotPointed = async(e_year,e_month,e_day,u_id)=>{
    ...
    EVENTS.loadDatabase((err)=>{
        if(err){
            console.log(`From eventApi.findEvents error in EVENTS.loadDatabase: ${err}`);
        };       
    });
    return(
        new Promise((resolve,reject)=>{           
            EVENTS.find({user_id:u_id, year:e_year, month:e_month, day:e_day, pointed:false},(err,events)=>{
                if(err){
...
const deleteOne = (search)=>{
    ...
    EVENTS.loadDatabase((err)=>{
        if(err){
            console.log(`From eventApi.deleteOne error in EVENTS.loadDatabase: ${err}`);
        };       
    });
